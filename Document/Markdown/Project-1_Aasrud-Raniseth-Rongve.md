...
Inserting the values of $a_{i}$, $b_{i}$ and $c_{i}$ into the general algorithm, we get this:
$$\\
\tilde{b}_{i}=1\\
\tilde{c}_{1}=-\frac{1}{2}\\
...
Its not fully apparent at small matrix sizes, but once they get big, the reduction in FLOPs makes a difference. The difference in time fluctuates a bit, but this is probably due to background tasks taking up some of the processing power, or reads/writes to the memory.
